...
final class HabitStore {
    ...
    /// Next `order` for a new habit (append to list).
    func nextHabitOrder() throws -> Int {
        var descriptor: FetchDescriptor<Habit> = FetchDescriptor<Habit>(
            sortBy: [SortDescriptor(\Habit.order, order: .reverse)]
        )
        descriptor.fetchLimit = 1
        if let top: Habit = try modelContext.fetch(descriptor).first {
            return top.order + 1
        }
        return 0
    }
    ...
    // Intentionally no "reset to demo data" in this build.
}
