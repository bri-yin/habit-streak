...
struct HomeView: View {
    ...
    @State private var habitEditorConfig: HabitEditorConfig?
    @State private var pendingHabitEditorConfig: HabitEditorConfig?
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .bottomTrailing) {
                ...
                FloatingActionButton(accessibilityLabelText: "Open habit library") {
                    guard habitEditorConfig == nil else { return }
        ...
        .sheet(isPresented: $showLibrarySheet, onDismiss: {
            if habitEditorConfig == nil {
                habitEditorConfig = pendingHabitEditorConfig
            }
            pendingHabitEditorConfig = nil
        }) {
            HabitLibraryView(
                onClose: { showLibrarySheet = false },
                onPickTemplate: { prefill in
                    showLibrarySheet = false
                    pendingHabitEditorConfig = HabitEditorConfig(existing: nil, prefill: prefill)
                },
                onCreateCustom: {
                    showLibrarySheet = false
                    pendingHabitEditorConfig = HabitEditorConfig(existing: nil, prefill: nil)
                }
            )
            .environment(store)
            .presentationDetents([.large])
        }
        .sheet(item: $habitEditorConfig) { config in
            HabitCreationSheet(
                existing: config.existing,
                prefill: config.prefill,
                onFinished: { habitEditorConfig = nil }
            )
            .environment(store)
            .presentationDetents([.large])
        }
        ...
        List {
            ForEach(scheduledHabits, id: \.id) { habit in
                HabitRowView(
                    ...
                    onEdit: { habitEditorConfig = HabitEditorConfig(existing: habit, prefill: nil) }
                )
            }
